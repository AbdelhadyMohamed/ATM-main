...
class _ApiBaseHelper {
  static Future<dynamic> httpSendRequest(http.BaseRequest request,RequestApi requestApi) async {
    http.StreamedResponse response;
    try {

      String deviceToken = "UPB1.230309.013";
      String deviceId = "";

      try {
        deviceId = await UniqueIdentifier.serial ?? deviceId;
      } catch (e) {}
      try {
  ...
  static Future<dynamic> _returnResponse(http.StreamedResponse response,RequestApi requestApi) async {
    ...
    switch (response.statusCode) {
      case 200:{
          if (jsonResponse["apiStatus"] == false) {
